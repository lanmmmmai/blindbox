...
const openRoomBox=onCall({region:REGION,cors:true,enforceAppCheck:false},async request=>{
  ...
  return db.runTransaction(async tx=>{
    const [rSnap,pSnap,bSnap,reqSnap,playersSnap]=await Promise.all([tx.get(rRef),tx.get(playerRef(code,uid)),tx.get(bRef),tx.get(reqRef),tx.get(rRef.collection("players").where("hasLeft","==",false))]);
    ...
    else if(room.gameMode==="normal"&&playersSnap.docs.reduce(sum=>sum+room.boxesPerPlayer,0)<=room.turnNumber){tx.update(rRef,{status:"completed",phase:"result",winnerUid:null,currentTurnUid:null,completedAt:now,updatedAt:now});finished=true;}
